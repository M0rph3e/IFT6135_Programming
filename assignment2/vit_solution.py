...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # ==========================
        e = torch.mean(inputs,-1,keepdim=True)
        #print("E : ", e)
        v = torch.var(inputs,-1,keepdim=True, unbiased=False)
        #print("Var : ", v)
        output = ((inputs-e)/torch.sqrt(v))*self.weight + self.bias
        #print("Out : ", output)
        return output
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length):
        ...
        self.hidden_state_linear = nn.Linear(d,3*d)
    ...
    def get_attention_weights(self, queries, keys):
        ...
        # TODO: Write your code here
        # ==========================
        a = torch.matmul(queries,keys.transpose(3,2))
        attention_weights = F.softmax(a/math.sqrt(self.head_size),dim=-1)
    ...
    def apply_attention(self, queries, keys, values):
        ...
        # TODO: Write your code here
        # ==========================
        weight_s = self.get_attention_weights(queries,keys)
        attended_values = weight_s.matmul(values)
    ...
    def split_heads(self, tensor):
        ...
        head = tensor.size()[:-1] # take head
        na = (self.num_heads, tensor.size(-1) // self.num_heads)
        new_shape = head + na
    ...
    def merge_heads(self, tensor):
        ...
        # TODO: Write your code here
        # ==========================
        a = tensor.permute(0, 2, 1, 3)
        #print("a  :", a.shape)
        #print("tensor : ", tensor.shape)
        new_a_shape = a.size()[:-2] + (a.size(-2) * a.size(-1),)
        #print("new a shape: ",new_a_shape)
        return a.reshape(*new_a_shape)
    ...
    def forward(self, hidden_states):
        ...
        # TODO: Write your code here
        # ==========================
        split_h = self.hidden_state_linear(hidden_states)
        split_h = self.split_heads(split_h)
        q,k,v = split_h.chunk(3, dim= -1)
        # print("Q : ", q,q.shape)
        # print("K : ", v,v.shape)
        # print("V : ", v,v.shape)
        att = self.apply_attention(q,k,v)
...
class PreNormAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # TODO: Write your code here
        # ==========================
        att  = self.layer_norm_1(x)
        att = self.attn(att)
        add = x + att
        ffn = self.layer_norm_2(add)
        ffn = self.linear(ffn)
        output = add + ffn
...
class VisionTransformer(nn.Module):
    ...
    def get_patches(self,image, patch_size, flatten_channels=True):
        ...
        patches_ = F.unfold(image,kernel_size=(patch_size,patch_size),stride=patch_size)
        #print("patches shape : ", patches_.shape)
        if flatten_channels:
            patches_ = patches_.flatten(2) #flatten over patches
        #print("patches shape : ", patches_.shape)
        return patches_.permute(0,2,1)
    ...
    def forward(self, x):
        ...
        # ==========================
        #print("X : ", x.shape)
        output =self.mlp_head(x[:,0]) #unconcatenate with x to retake cls_token
        #print("Output : ", output.shape)
        return output

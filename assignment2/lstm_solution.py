...
class LSTM(nn.Module):
    ...
    def forward(self, inputs, hidden_states):
        ...
        # ==========================
        x, h = self.lstm(self.embedding(inputs),hidden_states)
        clf = self.classifier(x) #classifier is a 2 layer MLP
        log_probas =F.log_softmax(clf,dim=-1)
        return log_probas,h
    ...
    def loss(self, log_probas, targets, mask):
        ...
        batch_size, sequence_length = log_probas.shape[0], log_probas.shape[1]
        nll = torch.zeros_like(targets, dtype= float)  
        predictedProb = torch.gather(log_probas,2,targets.unsqueeze(2))   #this avoid to loop through batches after discussing with others, It already put the right log probas (faster training) 
        nll = -predictedProb.squeeze() * mask
        t =  torch.count_nonzero(mask,-1) #t = torch.sum(mask,-1)
        t= t.unsqueeze(-1)
        loss_seq = (nll/t).sum(-1)
        loss = torch.mean(loss_seq)
        return loss

...
class SimSiam(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        """
        #assign z1 z2 to the encoder
        z1,z2 = self.encoder(x1), self.encoder(x2)
        #assign p1 p2 to the predictor
        p1,p2 = self.predictor(z1), self.predictor(z2)
        #check if stop gradient 
        if self.stop_gradient:
            #detach gradient of encoder
            z1 = z1.detach()
            z2 = z2.detach()
    ...
    def loss (self, p1,p2,z1,z2, similarity_function='CosineSimilarity'):
        ...
        COMPLETE ME. DONT MODIFY THE PARAMETERS OF THE FUNCTION. Otherwise, tests might fail.
        """
        sim_fctn = CosineSimilarity() if similarity_function=="CosineSimilarity" else CosineSimilarity() # because f**** putting string in parameters AHHHHHHHHH
        
        l = 0.5 * -torch.mean(sim_fctn(p1,z2)) #left
        r = 0.5 * -torch.mean(sim_fctn(p2,z1)) #right
        return l+r
...
class CosineSimilarity(Module):
    ...
    def forward(self, x1: Tensor, x2: Tensor) -> Tensor:
        ...
        COMPLETE ME. DONT MODIFY THE PARAMETERS OF THE FUNCTION. Otherwise, tests might fail.
        """
        x1_n = torch.norm(x1,p=2,dim=1)
        x2_n = torch.norm(x2,p=2,dim=1)
        norm_dot = x1_n* x2_n
        return bdot(x1,x2)/torch.maximum(norm_dot,torch.full_like(norm_dot ,fill_value=self.eps))